...
function getResolvedDatabaseUrl(urlValue: string | undefined): string | undefined {
  ...
  try {
    ...
    if (parsed.hostname === "host") {
      ...
      console.warn("[admin/prisma] DATABASE_URL host placeholder detected. Falling back to localhost for local development.");
...
export const prisma = globalThis.__adminPrisma
  ?? new PrismaClient(
    resolvedDatabaseUrl
