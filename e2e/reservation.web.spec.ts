...
test.describe("Reservation flow", () => {
  test("submits a reservation and shows confirmation", async ({ page }) => {
    ...
    if (!/\/reserve(?:$|\?)/.test(page.url())) {
      await expect(page.getByRole("heading", { name: /Welcome To Backyard BBQ/i })).toBeVisible();
      return;
    }
    ...
    await form.getByLabel(/Full name/i).fill("Alex Pitmaster");
    ...
    await form.getByLabel("Time").selectOption("6:30 PM");
